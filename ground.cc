                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Point3* S = new Point3();
    Point3* E = new Point3();
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.front(), S);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), E);
    Ray plane_ray = Ray(*S, *E - *S);
    Vector3 up = Vector3(0, 1, 0);
    Vector3 plane_normal = up.Cross(*E - *S);
    
    // 2. Project the 2D stroke into 3D so that it lies on the "projection plane"
    // defined in step 1.
    
    std::vector<Point3> projected_points;
    for (int i = 0; i < stroke2d.size() - 1; i++)
    {
        Point3 point_in_plane = Point3(0, 0, 0);
        float iTime;
        Point3 i_point = Point3(stroke2d.at(i).x(), stroke2d.at(i).y(), 0);
        Ray to_i_point = Ray(eye, eye - i_point);
        to_i_point.IntersectPlane(*S, plane_normal, &iTime, &point_in_plane);
        projected_points.push_back(point_in_plane);
    }
    /*Mesh m;
    m.SetVertices(projected_points);

    ground_mesh_.SetVertices(projected_points);
    ground_edge_mesh_.CreateFromMesh(m);*/
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        P[1] = hfunc(plane_normal, projected_points, P.ClosestPointOnPlane(*S, plane_normal));
